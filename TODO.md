...
- [ ] **地面下への カメラ潜り対策** (2026-03-09 Phase 3 以降に延期)
  - **理由**: CinemachineDeoccluder 実装による重処理のため、Phase 2.2 では見送り
  - **予定フェーズ**: Phase 3 以降（優先度: Medium）
...
### Naraku 落下検出・フォールバック改善

#### 背景

池や穴など DEM メッシュが存在しない地点にオブジェクトが落下した場合、
`GetClosestPointOnBounds` の 20 回 Raycast 試行がすべて失敗し、
池の上座標に配置されたまま復帰できない問題が起きていた。

#### 実装内容

**SpawnOriginTracker.cs (新規作成)**
- `Assets/Scripts/Core/Handlers/SpawnOriginTracker.cs`
- 各オブジェクトのスポーン原点を記憶する汎用 MonoBehaviour コンポーネント
- `SetSpawnOrigin(Vector3)` / `HasSpawnOrigin()` / `SpawnOrigin` プロパティ

**DemController.cs (変更)**
- `GetClosestPointOnBounds(Collider, out bool succeeded)` overload を追加
  - 20 回試行が全失敗したとき `succeeded = false` を返す
- `GetDemCenterSafePosition(float objectHeight)` を追加
  - DEM 中心座標の安全な代替位置を返す（全失敗時の最終フォールバック）
- `MaxIteration` プロパティ追加（ログ表示用）

**EnemyLitter.cs (変更)**
- `AgentJumpToStartPosition` で Warp 後に `SpawnOriginTracker.SetSpawnOrigin()` を呼び出し
- スポーン原点を Naraku フォールバック用に記録するようにした

**NarakuTriggerHandler.cs (変更)**
- `OnPlayerEnter` / `OnGenericObjectEnter` で `out bool succeeded` overload を使用
- `succeeded = false` のとき `GetFallbackPosition(other)` を呼び出す
- `GetFallbackPosition`: SpawnOriginTracker があれば SpawnOrigin、なければ DEM センターへ
- `AdjustYPositionByPlayerSpawn()` を追加（後述）
- `GetNarakuIndex()` を追加

#### PlayerArmature スポーン位置の登録

**EventLoader.cs (変更)**
- `Start()` に `RegisterPlayerSpawnOrigin()` を追加
- `PlayerArmature` に `SpawnOriginTracker` を付与し、シーン配置時の初期 Y 位置を記録
- プレイヤーが池に落ちた際の Naraku フォールバック帰還先として参照される

#### Naraku の Y 位置をプレイヤースポーン Y に追従

**背景**: プレイヤースポーン Y が高いステージで Naraku が遠すぎ、落下検出に時間がかかる問題。

**NarakuTriggerHandler.cs (変更)**
- `Awake()` から `AdjustYPositionByPlayerSpawn()` を呼び出し
- `PlayerArmature.transform.position.y` を基準に各 Naraku の Y を動的設定
  - Naraku   (idx=1): Y = playerSpawnY - 30
  - Naraku_1 (idx=2): Y = playerSpawnY - 60（加速後の補足用予備）

**NarakuController.cs (変更)**
- 前セッションで追加した Y 位置調整ロジックを削除
- アタッチされているコンポーネント（NarakuTriggerHandler）側に移した

#### 変更ファイル一覧

| ファイル | 変更種別 |
|---------|---------|
| `Assets/Scripts/Core/Handlers/SpawnOriginTracker.cs` | 新規作成 |
| `Assets/Scripts/Core/Handlers/NarakuTriggerHandler.cs` | 変更 |
| `Assets/Scripts/Game/GameManager/DemController.cs` | 変更 |
| `Assets/Scripts/Game/GameManager/NarakuController.cs` | 変更 |
| `Assets/Scripts/Game/Events/System/EventLoader.cs` | 変更 |
| `Assets/Scripts/Game/Units/Enemies/EnemyLitter.cs` | 変更 |
...
### �ꎞ�I�ȃo�b�N�A�b�v�e�X�g�t�@�C��

�ȉ��̃t�@�C���t�H���_�͊J���ߒ��Ő������ꂽ�ꎞ�t�@�C���ł���A**�A���t�@�Ō��J�O�ɍ폜���K�{**�ł��F

- [ ] \Assets/StreamingAssets/staging/ver2024/resources_backup_20260127_235817/\
  - **����**: �Â� Resources\staging �t�H���_����̃o�b�N�A�b�v�i��r�p�j
  - **�폜����**: �A���t�@�� v0.1.0-alpha �����[�X�O
  - **�o�b�N�A�b�v��**: ���ɏd�v�f�[�^�͊m�F�ς݁i��r���� 2026-01-28�j

- [ ] \Assets/Resources/staging/\
  - **����**: �Â��X�e�[�W�t�@�C���f�B���N�g���iStreamingAssets ���ŐV�Łj
  - **�폜����**: �A���t�@�� v0.1.0-alpha �����[�X�O
  - **�m�F**: �S�t�@�C���� StreamingAssets\staging �ɐV�����o�[�W����������

- [ ] \Assets/Scripts/UnitTest/YamlFileValidationTest.cs\
  - **����**: �J���p YAML �t�@�C�����؃e�X�g�X�N���v�g
  - **�폜����**: �A���t�@�Ń����[�X�O
  - **�A�[�J�C�u��**: \Assets/Scripts/Core/Editor/YamlFileValidationTest.cs\

- [ ] \Assets/Scripts/UnitTest/YamlCommandValidationTest.cs\
  - **����**: �J���p YAML �R�}���h�o���f�[�V�����e�X�g�X�N���v�g
  - **�폜����**: �A���t�@�Ń����[�X�O
  - **�A�[�J�C�u��**: \Assets/Scripts/Core/Editor/YamlCommandValidationTest.cs\
...
- [ ] \
esources_backup_20260127_235817\ �t�H���_���폜
- [ ] \Assets/Resources/staging/\ �f�B���N�g�����폜
- [ ] \Assets/Scripts/UnitTest/\ �t�H���_�����S�ɍ폜
- [ ] git commit �ł����̍폜���L�^

---

**�ŏI�X�V**: 2026-01-28
**�v���W�F�N�g**: OnoCoro (Unity 6.3 Geospatial Visualization)
