...
public class PlateauInfo : MonoBehaviour
{
    ...
    private GameObject GetInfoBox()
    {
        ...
        if (infoBox == null)
        {
            infoBox = Instantiate(Resources.Load("Prefabs/UI/InfoBox") as GameObject);
    ...
    private void DispInfo(Dictionary<string, string> dictInfo)
    {
        ...
        foreach (var pair in dictInfo)
        {
            ...
            // buf += $"key: {pair.Key}, value: {pair.Value}\n";
        ...
        buf += LangCtrl.GetLangVal("rebuildcost") +": "+ rebuildCost + "\n";
        ...
        txtRebuildText.text = " - " + rebuildCost.ToString() + "[BIT]";
    }
