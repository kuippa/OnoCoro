...
### Task 5: PlayMode 検証（ユーザー + エージェント・30分）[実施中 2026-06-16]

**1 回の通しプレイで W3 全体 + take5/take6 修正を確認する。**

ユーザー目視:
- [ ] 毎年の結果パネルに地震倒壊・火災延焼・無施策時予想・抑えた延焼・投資・ROI が表示される
- [ ] 施策を置いた年は火災延焼が baseline より減り、抑えた延焼・ROI が上がる
- [ ] 火災鎮火後に年が自動終了する（duration を待たずに結果へ）
- [ ] 3 年完走後に総括パネル → もう一度/タイトルへ
- [ ] Plaza が配置メニューに出ない／結果に避難カバー率が出ない
- [ ] ズームアウト時にカメラが真上を向かない（カメラ補正）
- [ ] 消火栓（狭30m）/防火水槽（広70m・高コスト）の使い分けの感触
- [ ] タイトル復帰で結果パネルが残らない／フォントが崩れない

エージェント（ログ解析）:
- [x] 数値がログ（DamageReportSystem）と一致。施策ありで Year2/3 の延焼を0に抑え ROI 6.7/21.7、
  施策なしは抑えた延焼ほぼ0（メカニクスは正しく機能）
- [x] カメラ補正ログ確認（BirdViewCamera +372 等の補正が走っている）
- [x] 既存ステージ（兼六園）の後方互換（ユーザー確認 OK・2026-06-16）

---

## W3 クローズ（2026-06-16）

Task 1-5 完了。**スケジュール上の W3 期限（6/28）を 12 日前倒しで完了**。
...
- 初期所有ポイント・配置可能ユニットの絞り込み（ユーザー「後ほど」）
- 消火栓/防火水槽の使い分けの意味づけ（現状は据え置き可）
- デモ動画撮影・YouTube・首長向けスライド（ユーザー作業）

**Task 5 結果（2026-06-16）**: 項目 1,3-6,8-10 は OK。残課題:
- BUG-S3-015: 高速ズームパンでカメラが数割反転（優先度低・W4 以降）
- BUG-S3-016: FireCube 傾き → 修正済み（FreezeRotation 全軸 + スポーン直立）
- ベースラインの run ぶれが大きい（Year3 無施策 26 vs 48）。施策なしでも「抑えた延焼>0」になる場合あり
- #7 ユーザー所見: ROI/抑えた延焼はスコアとして筋が良くないが、簡単な代替報酬が無いため
  予想延焼との差分で当面運用。ワークショップで多人数化すれば平均/中央値で精度向上の可能性
