...
### Task 5: PlayMode 検証（30 分）[2026-06-12 1回目実施・一部課題あり]

- [x] Year 1 Placement で開始 → Start Year → パターン由来イベントが発火 → duration 経過で停止
  （Editor.log で確認: パターン 3 件読み込み・3 年分登録・Year 1〜3 の開始/停止ログ）
- [x] 同一パターンを多重編成した年（Year 3）で全編成が発火する
  （Year 3 末の除去数 6 体 = 4 スポーン + 延焼分。多重編成の発火を確認）
- [x] Year 1 の残存敵が Year 2 開始時に存在しない（年末除去ログ: Year 2 末 2 体・Year 3 末 6 体）
- [x] Year 1 → 2 → 3 → Finished まで完走できる（「全年完走（Finished）」ログ確認）
- [x] Year 1 で配置したタワーが Year 2 でも残存している（2026-06-12 目視確認・
  プレイヤー配置の FireCube は年末に除去されることも確認）
- [x] `years` セクションの無い既存ステージが従来どおり動く（兼六園チュートリアルで確認）

**Task 5 完了（2026-06-12）**: チェックリスト 6 項目すべて合格。BUG-S3-001 は
「コードは正常・倒壊建物が遠方に分散し視認できない」という体験設計の問題と確定し、
W2 計画の論点（小マップ vs 広域消火ユニット vs 近傍優先スポーン）に引き継ぐ。

**検出された課題**（詳細: [../bugs.md](../bugs.md)・[../../notes_season3.md](../../notes_season3.md)）:

- BUG-S3-001（高）: 火災が視認できない。FireCube はスポーンしているが `random_doom_building` が
  倒壊 0 棟時に原点 (0,0,0) を返す疑い。診断ログ + random_position フォールバックを導入して再検証中
- BUG-S3-002（中）: 地震で床抜け（Season 2 由来・W2 以降で対応）
- BUG-S3-003（低）: YearPanel 表示崩れ（機能検証優先で意図的に保留）
- ゲームデザイン課題: WaterTurret 単体では木密の延焼を止められない。
  「小さいマップを作る」vs「広域消火ユニットを作る」は W2 計画の主要論点として持ち越し
