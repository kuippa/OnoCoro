...
**説明**: 海のうねり（波の大きさ・荒れ方）を変える

```yaml
value: <wind_speed>                          # 遠方風速。大きいほどうねりが高い
value: <wind_speed>, <duration>              # 秒数をかけてなめらかに変える
value: <wind_speed>, <duration>, <chaos>     # 波の荒れ方も変える
```

| パラメータ | 範囲 | 説明 |
|-----------|------|------|
| `wind_speed` | 0 - 250 | HDRP の遠方風速（km/h）。既定は 30。大きいほど波が高くなる |
| `duration` | 秒 | 目標値に達するまでの秒数。省略時は即座に変化 |
| `chaos` | 0.0 - 1.0 | 波の乱れ方。0 に近いほど規則的なうねり、1 に近いほど乱れる。即時反映 |

**例**:
```yaml
- time: 20
  event: swell
  value: 90, 15         # 15秒かけて荒れた海に

- time: 60
  event: swell
  value: 30, 20, 0.5    # 20秒かけて穏やかに戻す
```

[NOTE] うねりの変更は HDRP 側で波のシミュレーションが再計算される。
`duration` を指定した場合も毎フレームではなく一定間隔で更新している。
